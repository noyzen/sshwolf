...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-200 font-sans selection:bg-white/10">
      <div className={cn("h-10 flex items-end bg-zinc-950 border-b border-zinc-800 select-none titlebar w-full z-[60] overflow-hidden fixed top-0 left-0 right-0", isMac && "pl-20", isWindows && "pr-36")}>
        <div className="flex items-center h-full px-2 gap-1 overflow-x-auto w-full scrollbar-none">
          {serverSessions.map(session => (
            <div 
              ...
              className={cn(
                ...
                activeSessionId === session.id ? "bg-zinc-900 border-zinc-700 text-white z-10 font-medium shadow-sm" : "bg-zinc-900/50 border-transparent text-zinc-500 hover:bg-zinc-900 hover:text-zinc-300 mb-0.5"
              )}
            >
               <i className={cn("fa-solid fa-server text-[12px]", activeSessionId === session.id ? "text-white" : "opacity-50")} />
          ...
          <button onClick={() => { setEditingConnection(null); setManagerOpen(true); }} className="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-zinc-900 text-zinc-500 hover:text-white transition-colors mb-0.5 no-drag" title="New Connection"><i className="fa-solid fa-plus text-lg" /></button>
        </div>
      </div>
      ...
      <div className="flex-1 relative overflow-hidden bg-[#09090b] mt-10">
        ...
        {serverSessions.length === 0 && !isManagerOpen && (
           <div className="flex flex-col items-center justify-center h-full text-zinc-600">
              ...
              <button onClick={() => setManagerOpen(true)} className="mt-4 text-white hover:text-zinc-300 hover:underline">Open Connection Manager</button>
           </div>
        )}
      </div>
          ...
          <div className="space-y-3">
             <div className="grid grid-cols-1 gap-3">
                {connections.map(conn => (
                  <div key={conn.id} className="group bg-zinc-950 border border-zinc-800 hover:border-zinc-500/50 rounded-xl p-4 transition-all flex items-center justify-between">
                     <div className="flex items-center gap-4 cursor-pointer flex-1" onClick={() => createServerSession(conn)}>
                        <div className="w-10 h-10 rounded-lg bg-zinc-900 flex items-center justify-center text-white shadow-inner"><i className="fa-solid fa-server text-xl" /></div>
                        <div><h4 className="font-bold text-zinc-200">{conn.name}</h4><p className="text-xs text-zinc-500 font-mono">{conn.username}@{conn.host}</p></div>
                     </div>
                     <div className="flex gap-1">
                        <button onClick={() => setEditingConnection(conn)} className="p-2 text-zinc-500 hover:bg-zinc-900 hover:text-white rounded-lg transition-colors" title="Edit"><i className="fa-solid fa-pen-to-square text-base" /></button>
                        ...
                        <button onClick={() => createServerSession(conn)} className="ml-2 px-4 py-1.5 bg-zinc-100 hover:bg-white text-zinc-950 rounded-lg text-sm font-medium transition-colors shadow-lg shadow-zinc-500/10">Connect</button>
                     </div>
                  </div>
                ))}
                <button onClick={() => setEditingConnection({ port: 22 })} className="w-full py-3 border-2 border-dashed border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900/50 text-zinc-500 hover:text-zinc-300 rounded-xl transition-all flex items-center justify-center gap-2 font-medium"><i className="fa-solid fa-plus text-lg" /> Add New Connection</button>
             </div>
          </div>
        ) : (
          <form onSubmit={handleSaveConnection} className="space-y-4">
            <div><label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Connection Name</label><input required className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-white focus:ring-2 focus:ring-zinc-500/50 focus:border-zinc-500 outline-none transition-all placeholder:text-zinc-700" placeholder="Production Server" value={editingConnection.name || ''} onChange={e => setEditingConnection({...editingConnection, name: e.target.value})} /></div>
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2"><label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Hostname / IP</label><input required className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-white focus:ring-2 focus:ring-zinc-500/50 focus:border-zinc-500 outline-none transition-all placeholder:text-zinc-700 font-mono text-sm" placeholder="192.168.1.1" value={editingConnection.host || ''} onChange={e => setEditingConnection({...editingConnection, host: e.target.value})} /></div>
              <div><label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Port</label><input type="number" required className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-white focus:ring-2 focus:ring-zinc-500/50 focus:border-zinc-500 outline-none transition-all placeholder:text-zinc-700 font-mono text-sm" value={editingConnection.port || 22} onChange={e => setEditingConnection({...editingConnection, port: parseInt(e.target.value)})} /></div>
            </div>
            ...
            <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800 space-y-4">
                 <div className="flex items-center justify-between"><label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Authentication</label><div className="flex bg-zinc-900 p-0.5 rounded-lg border border-zinc-800"><button type="button" onClick={() => setAuthType('password')} className={cn("px-3 py-1 text-xs font-medium rounded-md transition-all", authType === 'password' ? "bg-zinc-100 text-zinc-950 shadow-sm" : "text-zinc-500 hover:text-zinc-300")}>Password</button><button type="button" onClick={() => setAuthType('key')} className={cn("px-3 py-1 text-xs font-medium rounded-md transition-all", authType === 'key' ? "bg-zinc-100 text-zinc-950 shadow-sm" : "text-zinc-500 hover:text-zinc-300")}>Private Key</button></div></div>
                 {authType === 'password' ? (<input type="password" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:ring-2 focus:ring-zinc-500/50 focus:border-zinc-500 outline-none transition-all placeholder:text-zinc-700 font-mono text-sm" placeholder="Password" value={editingConnection.password || ''} onChange={e => setEditingConnection({...editingConnection, password: e.target.value})} />) : (<div className="space-y-3"><div className="flex gap-2"><div className="relative flex-1"><i className="fa-solid fa-key absolute left-3 top-3 text-zinc-600 text-base" /><input readOnly className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 pl-10 text-zinc-300 focus:ring-2 focus:ring-zinc-500/50 focus:border-zinc-500 outline-none transition-all placeholder:text-zinc-700 font-mono text-xs" placeholder="No key selected" value={editingConnection.privateKeyPath || ''} /></div><button type="button" onClick={async () => { const path = await window.electron?.selectKeyFile(); if(path) setEditingConnection({...editingConnection, privateKeyPath: path}); }} className="px-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-sm border border-zinc-700 transition-colors">Browse</button></div><div className="relative"><i className="fa-solid fa-shield-halved absolute left-3 top-3 text-zinc-600 text-base" /><input type="password" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 pl-10 text-white focus:ring-2 focus:ring-zinc-500/50 focus:border-zinc-500 outline-none transition-all placeholder:text-zinc-700 font-mono text-sm" placeholder="Passphrase (Optional)" value={editingConnection.passphrase || ''} onChange={e => setEditingConnection({...editingConnection, passphrase: e.target.value})} /></div></div>)}
            </div>
            <div className="pt-4 flex justify-between items-center border-t border-zinc-800 mt-4"><button type="button" onClick={() => setEditingConnection(null)} className="text-zinc-500 hover:text-zinc-300 text-sm font-medium px-2 py-1">Back to list</button><button type="submit" className="px-6 py-2.5 bg-zinc-100 hover:bg-white text-zinc-950 rounded-lg font-medium shadow-lg shadow-zinc-500/10 transition-all">Save Connection</button></div>
          </form>
        )}
      </Modal>
    </div>
